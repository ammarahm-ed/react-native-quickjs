...
namespace qjs
{
  ...
  void QuickJSRuntime::loadCodeCache(CodeCacheItem *codeCacheItem, const std::string &url, const char *source, size_t size)
  {
  ...
  void QuickJSRuntime::updateCodeCache(CodeCacheItem *codeCacheItem, const std::string &url, const char *source, size_t size)
  {
    if (codeCacheDir_.empty())
    {
      return;
    ...
    std::string codeCachePath = codeCacheDir_ + "/" + cacheKey;
    std::vector<uint8_t> buffer(codeCacheItem->data, codeCacheItem->data + codeCacheItem
                                                                               ->size);
      ...
      const std::string &sourceURL)
  {
    ...
    const char *buffer_data = (const char *)buffer->data();
    size_t buffer_size = buffer->size();

    if (enableCodeCache)
    {
      CodeCacheItem codeCacheItem;
      loadCodeCache(&codeCacheItem, sourceURL, buffer_data, buffer_size);
      bool hasCodeCache = (codeCacheItem.result == CodeCacheItem::INITIALIZED);
      JSValue func, cachedFunc;
      __android_log_print(ANDROID_LOG_INFO, "QJSRUNTIME", "%zu", codeCacheItem.size);
      if (hasCodeCache)
      {
        LOG(ERROR) << "CODE CACHE LOADED";
      ...
      if (!hasCodeCache && !codeCacheDir_.empty())
      {
        size_t size;
        uint8_t *buf =
        ...
        if (buf && size != 0)
        {
          codeCacheItem.data = buf;
          codeCacheItem.size = size;
          codeCacheItem.result = CodeCacheItem::REQUEST_UPDATE;
          updateCodeCache(&codeCacheItem, sourceURL, buffer_data, buffer_size);
          //        free(codeCacheItem.data);
        }
        else
        {
      ...
      std::shared_ptr<jsi::HostObject> hostObject)
  {
    ...
    HostObjectProxy::RegisterClass(*this);

    JSValue object = JS_NewObjectClass(context_, HostObjectProxy::GetClassID());
      ...
      const jsi::String &name)
  {
    ...
    checkAndThrowException(context_);
    int tag = JS_VALUE_GET_TAG(prop);

    // __android_log_print(ANDROID_LOG_INFO, "QJSRUNTIME", "%s %s %d %s %d", "JS_VALUE_GET_TAG", jsName.c_str(), tag,"IS EQUAL", tag == JS_TAG_BOOL);
      ...
      jsi::HostFunctionType func)
  {
    ...
    HostFunctionProxy::RegisterClass(*this);

    JSValue global = JS_GetGlobalObject(context_);
    JSValue functionConstructor = JS_GetProperty(context_, global, JS_ATOM_Function);

    JSValue object =
        JS_NewObjectProtoClass(context_, JS_GetPrototype(context_, functionConstructor), jsClassID);

    ScopedJSValue scopedJsValue(context_, &object);
    ScopedJSValue scopedJsGlobal(context_, &global);
    ScopedJSValue scopedJsProto(context_, &functionConstructor);

    JS_SetOpaque(object, proxy->GetOpaqueData());
